...
pub struct Workflow {
    ...
    /// counter (PRINCIPLES "Single source of truth"). Omitted → that
    /// limit is unbounded. What ships is `max_depth: 5` and nothing
    /// else (ARCH §6 "No SPEND ships bounded; depth does"): the two
    /// spend ceilings stay off under the 2026-08-16 operator ruling,
    /// while depth — the tree's only prohibition on growth, and not a
    /// spend judgement — is declared. Declaring a ceiling is config an
