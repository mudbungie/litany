...
/// its dispatch commit (module docs), and nothing else.
...
/// docs): deletions and `summary/**` additions in `dispatch..tip`.
pub(super) fn product(
    ...
    Ok(Product {
        deletions: diff_class(parent_worktree, dispatch, compactor_ref, "D", None, git)?,
        summaries: diff_class(
            parent_worktree,
            dispatch,
            compactor_ref,
            "A",
