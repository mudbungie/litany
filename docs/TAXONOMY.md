...
- **Workflow mark** — `refs/litany/workflow/<agent-id>`: the *standing* per-agent ref naming the config commit whose `workflow.yaml` governs the agent from its next step boundary on, nearest mark on the agent's descent winning (ARCH §6 *The workflow mark*). Under follow-the-tip (bl-403b, §3 *Retarget and follow-the-tip vocabulary*) it is the per-agent override that wins over the followed tip — a switch, and equally the deliberate pin that holds one agent out of a lineage-wide change. Verb: `litany workflow <workspace> <agent> [--config <name> | --clear]`.
