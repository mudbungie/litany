...
- **Skill census** — the product of `litany skills <workspace> [--config <name>]`: one row per skill both homes offer, carrying its owner, its state (`active` / `unused` / `archived`), its last use and its last patch, ordered oldest-used first. Wholly derived at read time from git and the install pool — no counter, no store, no process — and printing ages rather than a `stale` verdict, a horizon being policy and policy being config.
