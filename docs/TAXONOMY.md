...
### Workflow, workflow mark, basic agentic loop (litany-specific stance)
**Field sense (Anthropic):** *workflows* are "systems where LLMs and tools are orchestrated through predefined code paths," contrasted with *agents*, where the LLM directs its own control flow (see §1 "Agent" above for the citation). Both are "agentic systems."

**litany's term of art (ARCH §6; operator ruling 2026-08-31, `docs/DESIGN_WORKFLOW_SWITCH.md`):** a **workflow** is the named, versioned declaration of what happens at every step — the event→action bindings, the compaction clock, the retry policy, budgets, the tool-output projection and the tool control: the contents of one config commit's `workflow.yaml`. The engine *operates by* workflows: a workflow determines the next step and nothing else, so it is consulted fresh at every step boundary and is **switchable on the fly** — the point being to test and optimize alternative workflows against each other (ARCH §9.3's experiments are workflow-config variants). The operator's 2026-08-31 coinage and litany's standing ARCH §6 term **converge — no new term is coined**; what the ruling added is the switchability requirement and the default's name. Reconciled here per the terminology discipline rather than coined silently. Two companion terms:

- **Workflow mark** — `refs/litany/workflow/<agent-id>`: the *standing* per-agent ref naming the config commit whose `workflow.yaml` governs the agent from its next step boundary on, nearest mark on the agent's descent winning (ARCH §6 *The workflow mark*). Under follow-the-tip (bl-403b, §3 *Retarget and follow-the-tip vocabulary*) it is the per-agent override that wins over the followed tip — a switch, and equally the deliberate pin that holds one agent out of a lineage-wide change. Verb: `litany workflow <workspace> <agent> [--config <name> | --clear]`; bare, that verb *reads* the derivation instead of writing it (bl-5c02).
- **Basic agentic loop** — the shipped default workflow, named: `template/workflow.yaml`'s declaration, frozen into every workspace's `config/default` by `litany new` and seeded under that name into the config-root workflow pool (`workflows/basic-agentic-loop.yaml`) by `litany prime`, from the same asset — dispatch → step loop → deliver results, intermediate compaction, retry, bounded tool output, nothing budget-bounded. An unmarked agent runs it through the identical pre-mark code path, so the name changes no behavior. ("Agentic loop" unqualified keeps the field sense in the previous entry; *the basic agentic loop*, definite article, is this named default.)
...
- **Compaction product** — what one pass makes, and the only thing its landing carries: the paths the pass nominated for deletion, the `summary/NNN.md` it wrote, and — since bl-e8ec — the **extract** the landing derives (below). Since bl-2071 the deletions are the union of the nominations and the **span sweep** (below), which the landing adds itself. Read off the compactor's branch **after its own dispatch commit** — everything at or before that commit is the tree it inherited — so the class is derived from git and needs no record of what the tools did. One definition, read twice: the landing applies it (`compactor::land::base`), and `mark_for_deletion` declines a nomination that falls inside it, because a pass's own product is not the branch's history to shed (ARCH §2.7).
...
- **Compaction in flight** — the state of a branch between dispatching a compactor and that compactor depositing its result: its checkpoint has fired and the answer has not come back. Derived, never stored — a compactor child with no `refs/litany/returned/<child>` mark — and a suppressor of the next checkpoint, so one span buys one pass (ARCH §2.7).
- **Retained tail** — the most recent commits kept out of the span, structurally outside the compactor's view: a commit count (`compaction.intermediate.keep_recent`) or, since bl-e8ec, a **token tail** (`keep_recent_tokens`) — the stretch of the transcript that cost at most that many provider-reported prompt tokens to append, read off successive model entries' `usage` (ARCH §2.3), never a tokenizer; one of the two, never both (`docs/DESIGN_CONTEXT_ECONOMY.md` §5.2).
- **Extract** — the deterministic compaction product: `summary/NNN.refs.md`, written by the landing (never by the model) from the transcript entries the compaction removes from context — verbatim user messages, error strings, pull-request numbers, commit shas, paths — under one byte cap (`compaction.intermediate.extract_bytes`; `docs/DESIGN_CONTEXT_ECONOMY.md` §5.3).
- **Span sweep** — the transcript entries a landing removes from the base without being asked: every `messages/**` path in the tree at the compaction point except the branch's dispatch entry, applied whenever the pass wrote a summary (`docs/DESIGN_CONTEXT_ECONOMY.md` §5.5, bl-2071). Which entries a summary replaces is the span, derived from git, not a judgement — so `mark_for_deletion` is left with the nominations that are judgements: a superseded work product, an earlier pass's summary. The retained tail is outside it by construction, since only the point's tree is read.
- **Soft archive** — the compactor's own ref, `agents/<id>-<compactor>`, on which the compaction span stays reachable after the landing squashes it out of the branch (ARCH §2.7, §9.2 reachability retention); what `search_history` reads (`docs/DESIGN_CONTEXT_ECONOMY.md` §4, §5.4).
- **Window trigger** — the `window_percent` checkpoint trigger: due when the branch's **last usage** (the `usage` sibling of the newest model entry, ARCH §2.3) fills `n` percent of the model's context window as brazen reports it in-band; declined loudly when the window is unknown (`docs/DESIGN_CONTEXT_ECONOMY.md` §5.1).
...
### Learning loop vocabulary (litany-specific coinage)
The post-response learning loop (`docs/DESIGN_LEARNING_LOOP.md`, bl-05e9) adopts hermes-agent's reviewer with staged writes. Four terms:

- **Reviewer** — the role dispatched by `worker_flush: dispatch(reviewer)` beside the compactor, off the same compaction point, granted `[read_file, apply_patch]`, keeping the inherited dialog as its subject (the third principled keeper, ARCH §2.2). It inspects the span about to be compacted for corrections, reusable techniques, failed or outdated loaded skills and packageable workflows, and edits workspace skills and the durable-facts document in its own tree. It writes no lineage.
- **Proposal** — one config commit on `proposal/<reviewer-id>`, parented on the followed config commit the reviewer read, minted by the `stage_proposal` action from the reviewer's own commits — the branch's transcript excluded, that being the executor's writing rather than the reviewer's — and refused whole when any path falls outside workspace skills and the facts document, when the lineage tip moved since the read, or when the config-authoring routine it is minted through refuses the commit (a pool-name collision, a malformed `SKILL.md`, a facts document over its cap). Written once, advanced by nobody; listed, accepted (a fast-forward of the lineage, refused when stale) or rejected by `litany proposal`. "Fresh" and "stale" are derived from the parent and the tip at read time, never stored.
- **Workspace skill** — a skill directory committed in a config lineage at `skills/<name>/`, as distinct from a pool skill under `<data-root>/skills/`; ownership is the path, names are unique across the two homes, and `skills/archived/<name>/` is the reversible archived state `litany skills` reports. Not to be confused with the branch-scoped `skills/<name>/` *loaded copy* on an agent branch, which is what a use is.
- **Skill census** — the product of `litany skills <workspace> [--config <name>]`: one row per skill both homes offer, carrying its owner, its state (`active` / `claimed` / `unused` / `archived`), its last use and its last patch, ordered oldest-used first. **`active` and a dated last use are one fact**: a skill is `active` when a living agent branch elected it, `claimed` when no branch did but a tool holds its name (so its description composes on every model call and no election can ever be recorded), `unused` when neither, `archived` when the body sits in the archive container. Wholly derived at read time from git and the install pool — no counter, no store, no process — and printing ages rather than a `stale` verdict, a horizon being policy and policy being config.

### Retarget and follow-the-tip vocabulary (litany-specific coinage)
An agent's governing *lineage* is derived from its branch's ancestry, and since the 2026-09-01 operator ruling (bl-403b, ARCH §2.2 *Fork chooses the lineage*) resolution **follows that lineage's current tip at every step boundary** — the **followed config commit** — so a config edit reaches every running conversation on the lineage at its next step. "Fork is the freeze" is the *retired* pre-ruling reading; fork now settles only which lineage governs. These terms name the act that remains.

- **Followed config commit** — the commit control resolves from: the governing lineage's current tip when exactly one config head stands over the governing commit (distinct-tip rule), else the fork commit itself, held loudly until a retarget settles diverged lineages (ARCH §2.2, `docs/DESIGN_CONFIG_FOLLOW.md`).
- **Retarget** — moving a *running* agent onto another config **lineage**, by re-forking its branch off that lineage's head and replaying its own history on top (**rebase-forward**, above). Not a merge, not a graft — and no longer how a same-lineage config edit reaches an agent (follow-the-tip does that by resolution alone): what retargets is the lineage, or the settling of a diverged one. The verb is `litany retarget <workspace> <agent> [--config <name>]`; a target the agent already resolves is a clean no-op.
...
### Inner invocation (litany-specific coinage)
litany coins the term for an invocation the model minted no wire id for (`docs/ARCHITECTURE.md` §3.3 *The program*, §3.4 `litany invoke`):
- **Inner invocation** — one `{name, input}` a *tool* raised rather than the model: the same shape a top-level `tool_use` block carries, run through the same depth refusal, grant gate, tool control and executor, with a diagnostic record of its own under a derived id (`<tool-id>-<k>`). It is not a wire-level tool invocation: the model minted no id for it, no per-invocation `tool_result` exists, and none of it enters the transcript — only the composing tool's own output does. A **program**'s calls are the shipped case (below); the `multi_tool` envelope was the first one, and retired into the program (`docs/DESIGN_CODE_EXECUTION.md` §5) — the term is widened, not replaced.
- **Multi-tool envelope** — *retired* with the built-in it named (bl-99bb). It meant one `multi_tool` invocation: a `tool_use` whose input carried the list of inner invocations plus execution metadata, written ahead of time. A program writes the same list as it runs, so the envelope is now the program itself; the entry stays here as the term's obituary, because transcripts that use it are immutable and still legible. Distinct from the ARCH §3.3 **result envelope**, which is the model-facing rendering of any one finished tool invocation and is very much alive.
...
### Program, stub module, door verb (litany-specific coinage, bl-acee)
Three terms for code execution (`docs/DESIGN_CODE_EXECUTION.md`), which widens **inner invocation** above rather than replacing it:
- **Program** — the python3 source one `python` built-in invocation runs, carried in the tool's input as `program`. A program's tool calls are inner invocations whose list is written as it runs; the multi-tool's was written ahead of time, and retires into the program.
- **Stub module** — `litany_tools`, the python module the built-in generates per program from the calling role's effective toolset: one keyword-only function per granted tool, parameters from the committed schema, docstring from the description, each one a `litany invoke` of that tool; plus the general `invoke(name, arguments)` those functions are sugar over, which is how a name python cannot spell is reached. Diagnostic — written beside the invocation's record, never into the worktree.
- **Door verb** — `litany invoke`, the front door for one inner invocation: a `tool_use` block on stdin, the raw result envelope on stdout, the tool's exit code. It is the tool window's gates at the command surface, and distinct from `litany tool <name>`, a tool's binary.
