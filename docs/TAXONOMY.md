...
- **Workflow mark** — `refs/litany/workflow/<agent-id>`: the *standing* per-agent ref naming the config commit whose `workflow.yaml` governs the agent from its next step boundary on, nearest mark on the agent's descent winning (ARCH §6 *The workflow mark*). The scoped sibling of the **retarget mark** (§3 *Retarget vocabulary*): a retarget moves the whole config by re-fork; the workflow mark moves the workflow fact alone and writes no branch. Verb: `litany workflow <workspace> <agent> [--config <name> | --clear]`.
