...
- **since bl-82d8** an exchange between two agents **terminates**. A terminal response is deposited into a peer's inbox only where a reply is *owed* — something delivered since this branch's previous terminal response asked for one — and a peer's reply asks nothing, so N deliberate `message` calls buy N answers and no more (ARCH §2.6 *A reply is owed once*). Before it, two cooperating subagents answered each other's answers forever: one `message` call, 333 transcript rows, 13.4M tokens. Pinned end to end by `prompt/tests/two_agent_exchange.rs`.
...
- **acceptance**
  ...
  - A create-only `<ws>/inbox/<agent>/<sender>-<NNN>.md` appears with `from:` and `deposited_at:` frontmatter — plus `from_name:` when the sender is an agent wearing a name — and the content as body; `<sender>` is `user` for a bare invocation.
...
- **scenario** Kill a runaway agent — it and everything it dispatched.
- **commands**
  - exec: `litany stop <ws> <agent>` (`--stop-children` is a retired, accepted spelling of the same act)
...
- **acceptance**
  ...
  - `mark_for_deletion` can remove and never write content (a `git rm`) — deletion-only structurally, so the worst case is lost compaction, never corrupted work. It declines, in-band, what the dispatch wrote and never rewrote: the branch's **dispatch entry** (`messages/001-…`), the conversation's opening prompt and so the goal in transcript form, and the system slot's `goal.md` / `soul.md` / `name`, which every model call on the branch is composed from. None of those are history, so none is compaction-eligible (ARCH §2.7). So a conversation that has been compacted still shows the prompt it was started with, and still has a goal, a soul and a name.
...
| ~~**G9**~~ | ~~**`--stop-children` has no integration proof.**~~ **RESOLVED (bl-3761).** The shipped walk held as specified — no behavior change was needed, only the missing evidence. `src/e2e/stop_children.rs` builds a live parent/child pair (both mid-model-call, distinct process groups). It proved both directions of the *then*-current rule: the flag fells the child, a bare stop does not. bl-3114 later retired the second direction — every stop takes the subtree — and the same file proves the replacement. Kept here as the record of the find. | See US-16. |
