...
cargo install brazen --version =0.0.14 --locked   # the provider adapter, always needed
...
cargo install brazen --version =0.0.14 --locked   # the pinned provider adapter
...
```
cargo install brazen --version =0.0.14 --locked
```
...
own version, `litany <version> (brazen 0.0.14)`. Its one home is the
...
(`brazen = "=0.0.14"`) for the canonical *types* only — the data plane
...
- **Auth and endpoints are brazen's.** Provider *rows* (endpoint,
  ...
  with `cargo install brazen --version =0.0.14`.
...
```
bz version "0.0.6" does not match the linked brazen crate "0.0.14"
```
