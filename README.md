...
cargo install brazen --version =0.0.17 --locked   # the provider adapter, always needed
...
cargo install brazen --version =0.0.17 --locked   # the pinned provider adapter
...
```
cargo install brazen --version =0.0.17 --locked
```
...
own version, `litany <version> (brazen 0.0.17)`. Its one home is the
...
(`brazen = "=0.0.17"`) for the canonical *types* only — the data plane
...
- **Auth and endpoints are brazen's.** Provider *rows* (endpoint,
  ...
  with `cargo install brazen --version =0.0.17`.
- **A failed model call names the row.** Which row litany routed a model call
  ...
  ```
  litany prompt: provider error (auth) on provider row "anthropic": no credential for
  provider `anthropic` … — no credential is reaching that row; authenticate it with
  ...
  The row is named twice because two layers know it for different
  reasons: brazen names its own resolved row (since 0.0.17, brazen
  bl-e809 — under `bz --serve` the row is picked by an inbound model
  string the client never saw), and the harness names the row it
  *routed* under, which is the `provider:` binding an operator edits.
...
```
bz version "0.0.6" does not match the linked brazen crate "0.0.17"
```
