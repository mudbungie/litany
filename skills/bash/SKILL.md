...
- **The command runs to completion.** No wall-clock limit is imposed,
  ...
  until the operator stops the agent. Bounding one is yours to do and
  the shell already expresses it: `timeout 30 <command>` ends it after
  thirty seconds and exits 124, which is why the tool takes no timeout
  parameter (a second home for a fact `timeout` already states).
